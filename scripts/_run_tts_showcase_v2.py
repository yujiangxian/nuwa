...
try:
    ...
    cv_paths = ['models/tts_models/cosyvoice_src/CosyVoice-main', 'models/tts_models/cosyvoice_src/CosyVoice-main/third_party/Matcha-TTS']
    for p in cv_paths:
        if p in sys.path:
            sys.path.remove(p)
...
# 4. OpenVoice (copy existing result if available)
print('>>> OpenVoice'); rg()
try:
...
# save
rp = RESULTS_DIR / 'tts_showcase_v2.json'
