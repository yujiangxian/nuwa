...
# 3. GLM-ASR
print('>>> GLM-ASR-Nano'); rg(); t0=time.time()
try:
    t50 = str(Path(TRANSFORMERS_50_PATH).resolve())
    if t50 not in sys.path: sys.path.insert(0, t50)
    ...
    results.append(R('GLM-ASR-Nano','ASR','success',text,lt,it,gm()))
    print(f'  OK: {text}')
except Exception as e:
...
# save
rp = RESULTS_DIR / 'asr_showcase.json'
