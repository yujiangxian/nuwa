#!/usr/bin/env python3
"""TTS 全模型效果测试 v4 (subprocess隔离GLM-TTS + 修复CosyVoice路径)"""
import sys, io
...
# 2. GLM-TTS (subprocess隔离)
print('>>> GLM-TTS'); rg(); t0=time.time()
try:
    script = RESULTS_DIR / '_temp_glm_tts_showcase.py'
    abs_audio = str(Path(TEST_AUDIO).resolve())
...
it=time.time()-t0
op='results/showcase/tts_glm.wav'
...
# save
rp = RESULTS_DIR / 'tts_showcase_v4.json'
