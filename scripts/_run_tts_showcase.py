...
try:
    sys.path.insert(0, 'models/tts_models/cosyvoice_src/CosyVoice-main')
    sys.path.insert(0, 'models/tts_models/cosyvoice_src/CosyVoice-main/third_party/Matcha-TTS')
    ...
    mp = 'models/tts_models/cosyvoice3'
    model = CosyVoice(mp, load_jit=False, load_onnx=False, load_trt=False)
...
try:
    ...
    if glm_dir not in sys.path: sys.path.insert(0, glm_dir)
    os.chdir(glm_dir)
    import torchaudio, soundfile as sf2
    def _pl(f, **kwargs):
        d, sr = sf2.read(f, dtype='float32')
...
# 4. OpenVoice
print('>>> OpenVoice'); rg(); t0=time.time()
try:
    sys.path.insert(0, 'external/openvoice')
    from openvoice import se_extractor
    from openvoice.api import ToneColorConverter
    base_speaker_tones = 'external/openvoice/base_speakers/ses/en-us.pth'
    tone_color_converter = ToneColorConverter('external/openvoice/checkpoints/converter/config.json', device='cuda')
    lt = time.time()-t0; rg(); t0=time.time()
    # OpenVoice 需要先有基础音频再用 tone color converter，完整流程较复杂
    # 简化为直接检查模型加载能力
    results.append(R('OpenVoice','TTS','failed','',lt,0,0,'需要额外基础音频生成流程'))
    print('  SKIP: 需要额外基础音频生成流程')
except Exception as e:
    results.append(R('OpenVoice','TTS','failed','',0,0,0,str(e)[:200]))
    traceback.print_exc()

# 5. Fish Speech
print('>>> Fish-Speech'); rg(); t0=time.time()
try:
    import fish_speech
    results.append(R('Fish-Speech','TTS','failed','',0,0,0,'缺少完整推理代码（audiotools/distributed 不兼容）'))
    print('  SKIP: 缺少完整推理代码')
except Exception as e:
...
# save
rp = RESULTS_DIR / 'tts_showcase.json'
