...
function PresetCard({ preset, cfg, onDownload }: { preset: PresetModel; cfg: { label: string; icon: typeof Cpu; color: string; bg: string; glow: string }; onDownload: (p: PresetModel) => void }) {
  const Icon = cfg.icon;
  const sizeText = formatSize(preset.size_mb);

  return (
    <div
      className="glass rounded-2xl p-5 glow-edge transition-all relative overflow-hidden group"
      style={{
        border: preset.is_downloaded ? `1px solid ${cfg.color}20` : '1px solid var(--border)',
        animation: 'slideInUp 0.35s ease',
      }}
    >
      {/* Downloaded indicator stripe */}
      {preset.is_downloaded && (
        <div className="absolute left-0 top-3 bottom-3 w-[3px] rounded-full" style={{ background: cfg.color }} />
      )}
      ...
      <div className={`flex items-start gap-4 ${preset.is_downloaded ? 'pl-2' : ''}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            {preset.is_downloaded && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(82,183,136,0.10)', color: '#52B788' }}>
                <Check size={9} /> 已下载
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-[10px] px-2 py-0.5 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(255,255,255,0.03)', color: 'var(--text-muted)' }}>
              <Database size={10} /> {sizeText}
            </span>
            <span className="text-[10px] px-2 py-0.5 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(255,255,255,0.03)', color: 'var(--text-muted)' }}>
              <Globe size={10} /> {preset.source}
            </span>
            {preset.size_mb > 5000 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(251,146,60,0.08)', color: '#FB923C' }}>
                <Zap size={9} /> 大模型
              </span>
            )}
            {preset.size_mb < 100 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(52,211,153,0.08)', color: '#34D399' }}>
                <Zap size={9} /> 轻量
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            {preset.is_downloaded ? (
              <span className="text-[10px] px-2 py-1 rounded-md font-medium flex items-center gap-1" style={{ background: 'rgba(82,183,136,0.08)', color: '#52B788' }}>
                <Check size={10} /> 已下载
              </span>
            ) : (
              <button
                onClick={() => onDownload(preset)}
                className="flex items-center gap-1.5 text-xs px-3.5 py-2 rounded-xl cursor-pointer transition-all font-medium"
                ...
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(72,202,228,0.06)'; }}
              >
                <Download size={14} /> 下载
              </button>
            )}
...
function ActiveModelBanner({
  ...
  // 动态列数：1-4 列自适应
  const gridCols =
    activeItems.length >= 4 ? 'md:grid-cols-4' :
    activeItems.length === 3 ? 'md:grid-cols-3' :
    activeItems.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-1';

  return (
    <div className="mb-6" style={{ animation: 'fadeIn 0.4s ease' }}>
      <div className="flex items-center gap-2 mb-3">
        <Sparkles size={14} style={{ color: '#D4AF37' }} />
        <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>当前活跃模型</span>
      </div>
      <div className={`grid grid-cols-1 ${gridCols} gap-3`}>
        {activeItems.map((item) => {
          ...
          return (
            <div
              key={item.type}
              className="glass rounded-2xl p-4 glow-edge transition-all relative overflow-hidden"
              style={{
            ...
            >
              ...
              <div className="flex items-center gap-3 pl-2">
                <div
                  ...
                  style={{ background: `${item.color}12`, border: `1px solid ${item.color}15` }}
                >
                  <Icon size={20} style={{ color: item.color }} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{model.name}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium flex items-center gap-1 shrink-0" style={{ background: `${item.color}12`, color: item.color }}>
                      <Check size={9} /> 使用中
                    </span>
                  </div>
                  <p className="text-[11px] mt-0.5 truncate" style={{ color: 'var(--text-secondary)' }}>
                    {item.label} · {formatSize(model.size_mb)} · {model.quant.toUpperCase()}
                  </p>
                </div>
              </div>
            </div>
...
function ModelCard({
  ...
  return (
    <div
      className="glass rounded-2xl p-5 glow-edge transition-all group relative overflow-hidden"
      style={{
        border: isCurrent ? `1px solid ${cfg.color}25` : '1px solid var(--border)',
        background: isCurrent ? `linear-gradient(135deg, ${cfg.bg}, transparent)` : undefined,
        animation: 'slideInUp 0.35s ease',
      }}
    >
      ...
      <div className={`flex items-start gap-4 ${isCurrent ? 'pl-2' : ''}`}>
        {/* Icon - clickable for detail */}
        <div
          onClick={() => onViewDetail(model)}
          className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-105 cursor-pointer"
          style={{
            background: `linear-gradient(135deg, ${cfg.color}15, ${cfg.color}05)`,
            border: `1px solid ${cfg.color}10`,
            boxShadow: isCurrent ? `0 0 16px ${cfg.glow}` : 'none',
          }}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <h3
              className="text-sm font-semibold truncate cursor-pointer transition-colors hover:opacity-80"
              style={{ color: 'var(--text-primary)' }}
              onClick={() => onViewDetail(model)}
            >{model.name}</h3>
            {isCurrent && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium flex items-center gap-1 shrink-0" style={{ background: `${cfg.color}12`, color: cfg.color }}>
                <Check size={9} /> 当前
              </span>
            )}
            {isOllama && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium shrink-0" style={{ background: 'rgba(123,130,225,0.08)', color: '#7B82E1' }}>Ollama</span>
            )}
            {lastUsedText && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium shrink-0 flex items-center gap-1" style={{ background: 'rgba(212,175,55,0.06)', color: '#D4AF37' }}>
                <Clock size={9} /> {lastUsedText}
              </span>
            )}
          </div>

          {/* Meta */}
          <p className="text-[11px] mb-1 truncate" style={{ color: 'var(--text-secondary)' }}>
            {model.description}
          </p>
          {/* Notes */}
          {notes && (
            <p className="text-[10px] mb-1 truncate italic" style={{ color: 'var(--text-muted)' }}>
              {notes}
            </p>
          )}
          {/* ID / Path hint for disambiguation */}
          <p className="text-[10px] mb-2 truncate font-mono" style={{ color: 'var(--text-muted)' }}>
            {model.id}
          </p>
          ...
          {model.main_files.length > 0 && (
            <div className="flex items-center gap-1.5 mb-3">
              <FileArchive size={11} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />
              <span className="text-[11px] truncate font-mono" style={{ color: 'var(--text-muted)' }}>
                {model.main_files[0]}{model.main_files.length > 1 && ` +${model.main_files.length - 1}`}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1">
              {!isCurrent && (
                <button
                  onClick={() => onSetCurrent(model.id)}
                  className="text-[11px] px-3 py-1.5 rounded-lg cursor-pointer transition-all font-medium"
                  style={{ color: cfg.color, background: `${cfg.color}08`, border: `1px solid ${cfg.color}15` }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = `${cfg.color}14`; }}
              ...
              <button
                onClick={() => onViewDetail(model)}
                title="查看详情"
                className="flex items-center justify-center rounded-lg transition-all hover:bg-white/5 cursor-pointer"
                style={{ width: 28, height: 28, color: 'var(--text-muted)', background: 'transparent', border: 'none' }}
              >
                <Info size={14} />
              </button>
              {!isOllama && (
                <button
                  ...
                  title="删除模型"
                  className="flex items-center justify-center rounded-lg transition-all hover:bg-red-500/10 cursor-pointer"
                  style={{ width: 28, height: 28, color: 'var(--text-muted)', background: 'transparent', border: 'none' }}
