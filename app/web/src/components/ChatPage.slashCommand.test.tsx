...
describe('斜杠激活与过滤 (Req 4.1, 4.2)', () => {
  it('输入 "/" 显示菜单，含 5 条内置命令 + 预设命令', () => {
    render(<ChatPage />);
    expect(screen.queryByTestId('slash-command-menu')).not.toBeInTheDocument();
    type('/');
    expect(screen.getByTestId('slash-command-menu')).toBeInTheDocument();
    // 5 builtin + 3 presets = 8 项。
    expect(screen.getByTestId('slash-command-item-7')).toBeInTheDocument();
    expect(screen.queryByTestId('slash-command-item-8')).not.toBeInTheDocument();
  });
...
describe('键盘导航与回绕 (Req 4.4, 4.5)', () => {
  it('ArrowDown 下移高亮，越过末项回绕到首项', () => {
    ...
    // ArrowUp 从首项回绕到末项：1 -> 0 -> 7。
    fireEvent.keyDown(input, { key: 'ArrowUp' }); // 1 -> 0
    fireEvent.keyDown(input, { key: 'ArrowUp' }); // 0 -> 7（回绕）
    expect(screen.getByTestId('slash-command-item-7')).toHaveAttribute('aria-selected', 'true');
    // ArrowDown 从末项回绕到首项：7 -> 0。
...
describe('预设插入与长度上限 (Req 5.1, 5.3, 4.7)', () => {
  it('点击预设命令插入其 content（Req 5.1, 4.7）', () => {
    render(<ChatPage />);
    type('/'); // p1 Greeting 在 index 5（5 条 builtin 之后）
  ...
  it('插入文本超 INPUT_MAX_LENGTH 时保持原文并提示（Req 5.3）', () => {
    render(<ChatPage />);
    type('/'); // p3 Long（2001 字符）在 index 7
    fireEvent.mouseDown(screen.getByTestId('slash-command-item-7'));
